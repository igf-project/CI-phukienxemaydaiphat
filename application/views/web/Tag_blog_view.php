					...
					<div class="row">
						<div class="col-md-9 col-md-offset-3">
							<ul class="breadcrumb">
								...
								<?php
								if(isset($parent_tag)){
									foreach ($parent_tag as $k => $v) {
										echo '<li><a href="'.base_url().$v['link'].'-'.$v['id'].'.html" title="'.$v['name'].'">'.$v['name'].'</a></li>';
									}
								}
								?>
								<li><a href="<?php echo base_url().$result['link'].'-'.$result['id'] ?>.html" title="<?= $result['name'] ?>"><?= $result['name'] ?></a></li>
							</ul>
						</div>
					</div>
				</div>
			...
			<div class="blog-product col-md-9">
				...
				foreach ($child_lv1 as $k => $v) {
					...
					<div class="tag-group-title">
						<h2><?= $v['name'] ?></h2>
						<div class="view-more">
							<a href="<?= base_url().$v['link'].'-'.$v['id']?>.html" title="<?= $v['name'] ?>">Xem tất cả »</a>
						</div>
					</div>
