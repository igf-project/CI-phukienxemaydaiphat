...
<div id="path">
	<ol class="breadcrumb">
		...
		<li class="active">Danh sách tags</li>
	</ol>
</div>
...
<table class="table table-striped table-bordered table-group" cellspacing="0" width="100%">
	...
	<tbody>
		...
		foreach ($result as $value) {
			...
			echo '<tr>
			<td>'.$i.'</td>
			<td class="tbl_actions">'.highlight_phrase($value['name'], $q, '<span style="color:red; font-weight: bold;">', '</span>').'
			...
			foreach ($value['child'] as $item) {
				echo '<li class="item-child child1">'.highlight_phrase($item['name'], $q, '<span style="color:red; font-weight: bold;">', '</span>');
			...
			echo '<td class="tbl_actions">';
			$i2 = 0;
			foreach ($value['child'] as $key => $item_child) {
				if(isset($item_child['child1'])){
					foreach ($item_child['child1'] as $item_child_child) {
						$i2++;
						echo '<li class="item-child child2 index-'.$key.'">'.highlight_phrase($item_child_child['name'], $q, '<span style="color:red; font-weight: bold;">', '</span>');
